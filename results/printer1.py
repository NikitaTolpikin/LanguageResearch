...
def AddWithoutMatches(placename, wdcnt, swcnt):
    with open('project_data.json', 'r', encoding='utf-8') as fh:
    ...
    if project_data.get(placename):
        ...
        place_dict = {placename: {'twit count': 1, 'word count': wdcnt, 'word rate': wdcnt,
                              'swear count': swcnt, 'swear rate': 0, 'mistake count': 0, 'mistake rate': 0, 'mistakes': {}}}
...
def AddWithMatches(placename, wdcnt, swcnt, mccnt, matches):
    with open('project_data.json', 'r', encoding='utf-8') as fh:
        project_data = json.load(fh)
    fh.close()
    mistakes = dict.fromkeys(matches, 1)
    ...
    if project_data.get(placename):
        ...
        place_dict = {placename: {'twit count': 1, 'word count': wdcnt, 'word rate': wdcnt, 'swear count': swcnt,
                                  'swear rate': 0, 'mistake count': mccnt, 'mistake rate': 0, 'mistakes': mistakes}}
    project_data.update(place_dict)
